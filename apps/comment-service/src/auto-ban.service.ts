...
export class AutoBanService {
  ...
  async evaluate(database: DatabaseExecutor, applicationId: string, memberId: string): Promise<void> {
    ...
    const state = await database.query<{ trigger_count: number; threshold_one: number; threshold_two: number; threshold_three: number; duration_one_hours: number; duration_two_hours: number; duration_three_hours: number }>(`SELECT COALESCE(offense.trigger_count, 0)::integer AS trigger_count, settings.auto_ban_threshold_one AS threshold_one, settings.auto_ban_threshold_two AS threshold_two, settings.auto_ban_threshold_three AS threshold_three, settings.auto_ban_duration_one_hours AS duration_one_hours, settings.auto_ban_duration_two_hours AS duration_two_hours, settings.auto_ban_duration_three_hours AS duration_three_hours FROM application_settings settings LEFT JOIN user_offenses offense ON offense.application_id = settings.application_id AND offense.member_id = $2 WHERE settings.application_id = $1`, [applicationId, memberId]);
    ...
    const durationHours = count === 1 ? current.duration_one_hours : count === 2 ? current.duration_two_hours : count === 3 ? current.duration_three_hours : null;
    await database.query(`INSERT INTO user_blocks (application_id, member_id, mode, source, expires_at) VALUES ($1, $2, $3, 'auto', CASE WHEN $4::integer IS NULL THEN NULL ELSE now() + ($4::integer * interval '1 hour') END) ON CONFLICT (application_id, member_id) DO UPDATE SET mode = EXCLUDED.mode, source = 'auto', expires_at = EXCLUDED.expires_at WHERE user_blocks.source = 'auto' OR user_blocks.expires_at <= now()`, [applicationId, memberId, mode, durationHours]);
